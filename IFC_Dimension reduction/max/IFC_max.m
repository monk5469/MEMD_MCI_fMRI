% This file execute a maximum-inter-class-distance dimension reduction 
...
for ind = 1 : size(NumSub_MCI,1)  
  ...
  %%% via the  maximum-inter-class-distance criterion. %%%%%%%%%%%
 ...
 %%%%%%%% 2. Obtain the dimention redcetion matrix in a NC Group %%%%%%%%%    
    for nSub=1:length(NumSub_NC(ind,:))
        ...
        NC_max(:,:,nSub)=RHO_index;
    end      
end

%%%% FDR Correction %%%
MCI_max_addfar=corr_add_fdr(mean(MCI_max,3),getFDR(CmpPValPearson(mean(MCI_max,3),ROI_timepoint)));
NC_max_addfar=corr_add_fdr(mean(NC_max,3),getFDR(CmpPValPearson(mean(NC_max,3),ROI_timepoint)));
save('NCgroup_IFC_max','NC_max_addfar');
save('MCIgroup_IFC_max','MCI_max_addfar');